...
export const FOOD_REACTION_OPTIONS: ReactionOption[] = [
...
export function FoodReactionBarPresentation({
    summaries = [],
    currentUserReaction,
    ...
    return (
        <div className="mt-4 flex flex-wrap gap-2 border-t border-white/10 pt-4">
            {FOOD_REACTION_OPTIONS.map((option) => {
                    ...
                    <button
                        ...
                        title={
                            renderReactionAction
                                ...
                                : `${option.label} (${option.valueLabel}) — available on web`
                        }
                    >
                ...
                return renderReactionAction ? (
                    <div key={option.type} className="contents">
                        {renderReactionAction(option, action)}
                    </div>
                ) : (
                    <div key={option.type}>{action}</div>
                );
            })}
        </div>
    );
