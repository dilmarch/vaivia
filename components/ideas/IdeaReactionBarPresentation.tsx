...
function ReactionAvatarStack({ profiles }: { profiles: IdeaReactionProfile[] }) {
  ...
  return (
    <span className="flex items-center">
      {profiles.slice(0, 3).map((profile, index) => (
        ...
        >
          {profile.avatar_url ? (
            // Shared browser presentation must support both Next.js and Capacitor.
...
export function IdeaReactionBarPresentation({
  summaries = [],
  currentUserReaction,
  compact = false,
  renderAction,
  ...
  return (
    ...
    >
      {REACTION_OPTIONS.map((option) => {
        ...
        const className = `inline-flex min-h-9 items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-bold uppercase tracking-[0.16em] transition duration-300 hover:-translate-y-0.5 ${
          isSelected
        ...
        const actionProps: ReactionActionProps = {
          children,
          className,
        ...
        return renderAction ? (
          renderAction(option.type, actionProps)
        ) : (
          <button
            key={option.type}
            type="button"
            disabled
            aria-label={`${option.label} voting unavailable`}
            {...actionProps}
