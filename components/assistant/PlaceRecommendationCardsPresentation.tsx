...
export type PlaceActionReviewRenderProps = {
    tripId: string;
...
export type SavedTargetRenderProps = {
...
export default function PlaceRecommendationCards({
    ...
    renderActionReview?: (props: PlaceActionReviewRenderProps) => ReactNode;
    ...
    const hasActionContext = Boolean(tripId && conversationId && messageId);
    const canAct = hasActionContext && actionsEnabled && Boolean(renderActionReview);
    ...
    return (
                        ...
                        {savedTargets.length > 0 ? (
                            <div className="mt-3 flex flex-wrap gap-2">
                                {savedTargets.map((target) => {
                                    ...
                                    const className =
                                        "rounded-full border border-lime-300/20 bg-lime-300/[0.08] px-2.5 py-1 text-[10px] font-black text-lime-200 underline-offset-2 hover:underline";
                                    return renderSavedTarget ? (
                        ...
                        place.liveDetailsAvailable !== false ? (
                            <div className="relative mt-2">
                                <button
                                    ...
                                    className="inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/[0.055] px-3 py-2 text-xs font-black text-white transition hover:bg-white/[0.1] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-lime-300 disabled:cursor-not-allowed disabled:opacity-50"
                                >
                                    <Save className="h-3.5 w-3.5" aria-hidden="true" />
                                    Save
                                    <ChevronDown className="h-3.5 w-3.5" aria-hidden="true" />
                                </button>
                                {canAct && openMenuId === place.recommendationId ? (
                                    <div
                                        role="menu"
        ...
        {pendingAction && tripId && conversationId && messageId && renderActionReview
            ? renderActionReview({
                tripId,
                conversationId,
                messageId,
