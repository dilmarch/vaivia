  ...
  if (uniqueYears.length > 8) {
    ...
    return (
      <span className="block leading-5">
        ...
        <span className="block">{uniqueYears.length} times visited</span>
      </span>
    );
  }
...
export default function ScratchMap({
  ...
  async function handleCountryClick(
    ...
    if (isManuallyScratched) {
      ...
      if (onScratchCountryChange) {
        try {
          await onScratchCountryChange(countryCode, false);
        } catch (caught) {
          error = caught;
        }
      } else {
        const supabase = createClient();
        // This table exists in production but is not present in the generated
        // client schema shipped by this checkout yet.
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const result = await (supabase.from as any)(
    ...
    if (onScratchCountryChange) {
      try {
        await onScratchCountryChange(countryCode, true);
      } catch (caught) {
        error = caught;
      }
    } else {
    ...
    <section className={`space-y-5 ${className}`}>
      <div className={statsClassName || "grid gap-3 md:grid-cols-3"}>
        <div className="rounded-[1.25rem] border border-white/10 bg-white/[0.06] p-4 shadow-xl shadow-black/20">
          ...
          <p className="mt-2 text-3xl font-black text-white">{visitedCount}</p>
        </div>
        <div className="rounded-[1.25rem] border border-white/10 bg-white/[0.06] p-4 shadow-xl shadow-black/20">
          <p className="text-xs font-black uppercase tracking-[0.18em] text-lime-200">
            World explored
          </p>
          <p className="mt-2 text-3xl font-black text-white">{percentage}%</p>
        </div>
        <div className="rounded-[1.25rem] border border-white/10 bg-white/[0.06] p-4 shadow-xl shadow-black/20">
          <p className="text-xs font-black uppercase tracking-[0.18em] text-lime-200">
            Continents explored
      ...
      {visitedCount === 0 ? (
        <div className="rounded-[1.5rem] border border-lime-300/20 bg-lime-300/[0.08] p-5 text-white shadow-xl shadow-black/20">
          ...
          <p className="mt-2 max-w-2xl text-sm font-semibold leading-6 text-slate-300">
            Add countries to your Passport Stamps in Settings and they’ll appear
            here.
          </p>
          <a
            href={settingsHref}
            ...
            {showScratchHint ? (
              <div className="absolute left-0 top-[calc(100%+0.75rem)] w-64 rounded-2xl border border-lime-300/25 bg-slate-950/95 p-4 text-sm font-bold leading-6 text-white shadow-2xl shadow-black/50">
                ...
                <p className="mt-1 text-xs font-semibold text-slate-300">
                  Activate scratch mode to scratch off countries you have
                  visited.
                </p>
              </div>
            ) : null}
            ...
            <defs>
              ...
              >
                <animateTransform
                  ...
                  additive="sum"
                />
                <rect width="34" height="24" fill="rgba(15,23,42,0.88)" />
                <path
              ...
              <Geographies geography={GEO_URL}>
                {({ geographies }) =>
                  geographies.map((geo: GeographyRecord) => {
                    ...
                    const isScratching =
                      country.visited && scratchingCode === country.code;
                        ...
                        <Geography
                          ...
                          style={{
                            default: {
                              ...
                              strokeWidth: country.visited ? 0.7 : 0.5,
                              outline: "none",
          ...
          >
            ...
            <p className="mt-1 text-xs uppercase tracking-[0.16em] text-lime-200">
              {hoveredCountry.passportStamped ? (
                <VisitedYearsDisplay years={hoveredCountry.visitedYears} />
          ...
          <>
            ...
            <p className="mt-1 text-sm font-bold text-lime-200">
              {activeCountry.passportStamped ? (
                <VisitedYearsDisplay years={activeCountry.visitedYears} />
