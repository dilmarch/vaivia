...
import { MobileSocialProfileSections } from "../components/MobileSocialProfileSections";
...
export function ProfileScreen({
  ...
  if (isLoading && !profile) {
    return (
      <main className="mx-auto max-w-4xl px-4 py-8 text-white" aria-busy="true">
        Loading profile…
      </main>
    );
  }
    ...
    return (
      <main className="mx-auto max-w-4xl px-4 py-8">
        <ScreenMessage
          ...
          message={error || "VAIVIA could not load your profile."}
    ...
    <main className="mx-auto max-w-4xl space-y-5 px-4 py-6 text-white">
      <section className="overflow-hidden rounded-[2rem] border border-white/10 bg-[#050712] shadow-2xl shadow-black/30">
        <ProfileHeaderPresentation
          ...
          editAction={
            <button
              ...
              className="inline-flex min-h-11 items-center gap-2 rounded-full bg-lime-300 px-5 py-2.5 text-sm font-black text-slate-950"
            >
          ...
          signOutAction={
            <Button
              ...
              className="border-white/10 bg-white/[0.08] text-slate-100"
            >
        ...
        <div className="space-y-5 p-5 sm:p-6">
          {status ? (
            <p
              className="rounded-2xl border border-lime-300/25 bg-lime-300/10 p-4 text-sm font-bold text-lime-100"
              role="status"
            >
              {status}
            </p>
          ...
          {error ? (
            <p
              className="rounded-2xl border border-red-300/25 bg-red-500/10 p-4 text-sm font-bold text-red-100"
              role="alert"
            >
              {error}
            </p>
          ...
          {isEditing ? (
            <div className="space-y-4">
              <form
                ...
                className="flex flex-wrap items-end gap-3 rounded-[1.5rem] border border-white/10 bg-white/[0.06] p-5"
              >
                <label className="min-w-0 flex-1">
                  ...
                  <input
                    ...
                    required
                    className="mt-2 block w-full text-sm font-bold text-slate-300"
                  />
                </label>
                <Button
                  type="submit"
                  disabled={isSaving}
                  className="min-h-11 rounded-full bg-lime-300 font-black text-slate-950"
                >
                  Upload
                </Button>
              </form>
                ...
                {[
                  ...
                  <label key={name} className="block">
                    ...
                    <input
                      ...
                      defaultValue={value}
                      disabled={name === "email" && !profile.canChangeEmail}
                      required={name === "username" || name === "email"}
                      className="mt-2 h-12 w-full rounded-xl border border-white/10 bg-slate-950 px-3 text-sm font-bold text-white disabled:opacity-55"
                    />
                  </label>
                ))}
                <Button
                  type="submit"
                  disabled={isSaving}
                  className="h-12 rounded-full bg-lime-300 font-black text-slate-950 sm:col-span-2"
                >
          ...
          <button
            type="button"
            onClick={onSettings}
            className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-full border border-white/10 bg-white/[0.06] px-5 text-sm font-black text-white"
          >
            <Settings className="h-4 w-4" aria-hidden="true" /> Account settings
          </button>
        </div>
      </section>
      <MobileSocialProfileSections
        apiClient={apiClient}
        userId={profile.id}
        onFriend={onFriend}
      />
