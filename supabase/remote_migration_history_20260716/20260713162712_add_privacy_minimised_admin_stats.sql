...
comment on table public.user_activity_daily is
  'Privacy-minimised daily authenticated activity used for aggregate product analytics. One row per user per UTC day.';
...
drop policy if exists "Users can record their own daily activity" on public.user_activity_daily;
...
drop policy if exists "Users can refresh their own daily activity" on public.user_activity_daily;
