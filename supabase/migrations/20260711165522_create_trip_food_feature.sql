...
create table public.trip_food_items (
  ...
  constraint trip_food_items_item_type_check check (
    item_type = any (array['place'::text, 'food'::text])
  ),
  ...
  constraint trip_food_items_place_required check (
    ...
    or (
      ...
      and length(btrim(formatted_address)) > 0
    )
  ),
...
create table public.trip_food_reactions (
  ...
  constraint trip_food_reactions_item_match foreign key (trip_id, food_item_id)
    references public.trip_food_items(trip_id, id)
    on delete cascade,
  constraint trip_food_reactions_one_per_user unique (food_item_id, user_id),
  constraint trip_food_reactions_reaction_check check (
    reaction = any (array['heart'::text, 'thumbs_up'::text, 'thumbs_down'::text])
  )
);
...
create index trip_food_items_trip_type_idx
on public.trip_food_items(trip_id, item_type);

create index trip_food_items_google_place_idx
on public.trip_food_items(trip_id, google_place_id)
where google_place_id is not null;

create index trip_food_items_created_by_idx
on public.trip_food_items(created_by);

create index trip_food_reactions_trip_idx
on public.trip_food_reactions(trip_id);

create index trip_food_tried_trip_idx
on public.trip_food_tried(trip_id);
...
using (
  ...
  or exists (
    select 1
    from public.trips
    where trips.id = trip_food_items.trip_id
